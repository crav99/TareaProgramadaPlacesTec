...
class ComboKeyHandler extends KeyAdapter {
  ...
  @Override public void keyTyped(final KeyEvent e) {
    EventQueue.invokeLater(new Runnable() {
      @Override public void run() {
        String text = ((JTextComponent) e.getComponent()).getText();
  ...
  private static void setSuggestionModel(
    ...
    ((JTextComponent) comboBox.getEditor().getEditorComponent()).setText(str);
  }
  private static ComboBoxModel<String> getSuggestedModel(List<String> list, String text) {
    DefaultComboBoxModel<String> m = new DefaultComboBoxModel<>();
    String place = "https://maps.googleapis.com/maps/api/place/autocomplete/json?input="+text+"&types=geocode&key="+"";
    ...
    AutocompletePrediction[] request;
      try {
          request = PlacesApi.placeAutocomplete(context, text).await();
      } catch (ApiException | InterruptedException | IOException ex) {
          Logger.getLogger(ComboKeyHandler.class.getName()).log(Level.SEVERE, null, ex);
          return m;
      }
    for (AutocompletePrediction s : request) {
        m.addElement(s.structuredFormatting.mainText);
    }
    return m;
  }
}
